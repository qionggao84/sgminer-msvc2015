...
//	cgpu->thread_concurrency = cgpu->thread_concurrency/4;
...
out:
  ...
  if (algorithm->type == ALGO_LBRY) {
	  ...
	  clState->buffer1 = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_uint) * 8 * GlobalThreads, NULL, &status);

	  if (status != CL_SUCCESS) {
		  applog(LOG_ERR, "Error %d when creating LBRY state buffer.\n", status);
		  return NULL;
	  }
...
//	  readbufsize = 76UL;
	...
	if (algorithm->type == ALGO_LYRA2Z330) {
	  size_t GlobalThreads;
	  //	  readbufsize = 76UL;
	  ...
	  if (status != CL_SUCCESS) {
		  applog(LOG_ERR, "Error %d when creating lyra2z330 state buffer.\n", status);
		  return NULL;
	  }


	  clState->Scratchpads = clCreateBuffer(clState->context, CL_MEM_READ_WRITE, sizeof(cl_ulong)*LYRA2Z330_SCRATCHBUF_SIZE * GlobalThreads, NULL, &status);

	  if (status != CL_SUCCESS) {
		  applog(LOG_ERR, "Error %d when creating lyra2z330 scratchpads buffer.\n", status);
		  return NULL;
	  }
  }
